...
/* CONTROLS */
double rkProp       = 0.0;     // right motor proportional gain
double rkInteg      = 0.0;      // right motor integral gain
double rErrorRange  = PI/8;    // right motor error range

double lkProp       = 0.0;     // left motor proportional gain
...
/**************************************** DEMO VARIABLES ****************************************/
double desiredXPos      = 00;           // [in]
double phi_des          = 00;           // [rad]
...
void setup() {
  ...
  /* choosing gain values according to distance to move - gain values experimentally found */
  ...
  if (leftFix) {                      // fix for left encoder channels initializing to weird values when left encoder is plugged into motor shield
    ...
    L_angVelPrev = 0.0;               // this is the fix that the TA suggested we implement    
    L_angVelNow = 0.0;
    L_linVel = 0.0;
  }
